...
**严禁隐式子任务**：
1. **扁平化原则**：严禁在执行过程中创建脱离主列表的“临时子任务”。若任务复杂，**必须**在主 Todo 列表中将其显式拆分为多个具体步骤。
2. **单一列表**：始终维护**唯一的** Todo 列表。不要同时维护“主任务”和“当前任务”两套逻辑。
3. **完成即检查**：每完成一项任务，**必须**重新审视 Todo 列表。只要列表中仍有 `pending` 状态的项目，**严禁**结束会话或停止执行。
