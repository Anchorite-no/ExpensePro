...
1. **允许拆分**：复杂任务应拆解为子任务列表。
2. **强制回归 (Pop Stack)**：子任务全部完成后，**严禁**直接结束。必须立即重新加载/审查**主任务列表**。
3. **最终核对**：只有当**主任务列表**中所有条目均为 `completed` 时，方可交付任务。
4. **禁止“只见树木”**：不要迷失在子任务中，时刻通过 `TodoWrite` 保持对全局进度的感知。
