...
import { Settings2, X, DollarSign, Target, ArrowUp, ArrowDown, Trash2, PlusCircle, Plus } from "lucide-react";
...
export function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  try {
    ...
    if (raw) {
      categories = typeof raw === 'string' ? JSON.parse(raw) : raw;
    }
  } catch (e) {}

  let budget = { daily: 0, weekly: 0, monthly: 0 };
  try {
    const raw = settings?.budgetConfig;
    if (raw) {
  ...
  const saveCategories = (newCats: Record<string, string>) => {
    updateSettings({ ...settings, categories: JSON.stringify(newCats) });
  };
    ...
    <div className="category-manager-overlay" onClick={onClose}>
      <div className="category-manager settings-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="settings-tabs">
          <button className={`settings-tab ${activeTab === "general" ? "active" : ""}`} onClick={() => setActiveTab("general")}>通用设置</button>
          <button className={`settings-tab ${activeTab === "categories" ? "active" : ""}`} onClick={() => setActiveTab("categories")}>分类管理</button>
        </div>
        ...
        {activeTab === "general" && (
          <div className="settings-general">
            <div className="setting-group">
              ...
              <div style={{ width: 200 }}>
                <Select value={currency} onChange={saveCurrency} options={CURRENCIES} />
              </div>
            </div>
            <div className="setting-group">
