...
import {
  ...
  Download, Pencil, X, Check, CheckCheck
} from "lucide-react";
...
interface Props {
  ...
  onEdit: (id: number, title: string, amount: number, category: string, date: string) => void;
  currency: string;
}

export default function TransactionsPage({ expenses, categories, onDelete, onAdd, onEdit, currency }: Props) {
  ...
  const [form, setForm] = useState({ title: "", amount: "", category: defaultCategory, date: "" });
  ...
  const [editForm, setEditForm] = useState<EditDraft>({ title: "", amount: "", category: "", date: "" });
  ...
  const handleAdd = () => {
    ...
    setForm({ title: "", amount: "", category: defaultCategory, date: "" });
    setShowForm(false);
  };
  ...
  const saveSingleEdit = () => {
    ...
    onEdit(editingId, editForm.title, Number(editForm.amount), editForm.category, editForm.date);
    cancelSingleEdit();
  };
  ...
  const saveBatchEdit = () => {
    ...
    Object.entries(editDrafts).forEach(([idStr, draft]) => {
      ...
      if (changed) {
        onEdit(id, draft.title, Number(draft.amount), draft.category, draft.date);
      }
    });
    cancelBatchEdit();
  };
