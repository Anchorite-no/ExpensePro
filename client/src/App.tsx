...
import {
  ...
  ArrowUp, ArrowDown, LogOut, DollarSign, Target
} from "lucide-react";
...
const loadBudget = (): BudgetConfig => {
  try {
    const saved = localStorage.getItem("budget_config");
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
...
function AppContent() {
  ...
  // 预算（日/周/月）
  const [budget, setBudget] = useState<BudgetConfig>(loadBudget);
  ...
  const moveCategory = (index: number, direction: "up" | "down") => {
    ...
    entries.forEach(([key, val]) => { newCategories[key] = val; });
  ...
  const saveBudgetField = (field: keyof BudgetConfig) => {
    ...
    setBudget(updated);
    localStorage.setItem("budget_config", JSON.stringify(updated));
  ...
  const editExpense = async (id: number, title: string, amount: number, category: string, date: string) => {
    if (!token) return;
    try {
                  ...
                  {Object.entries(categories).map(([name, color], index, arr) => (
                    <div key={name} className="category-item">
                      ...
                      <div className="category-actions">
                        ...
                        <button className="action-btn delete" onClick={() => removeCategory(name)} title="删除" disabled={categoryList.length <= 1}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
